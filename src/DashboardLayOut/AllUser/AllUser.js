...
const AllUser = () => {
    ...
    const { data: users = [], refetch, isLoading } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://furniture-server-site.vercel.app/users');
            const data = await res.json();
            return data;
        }
    })
    const handleVerifyBtn = (id) => {
        fetch(`https://furniture-server-site.vercel.app/users/${id}`, {
            method: 'PUT',
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleDeleteProduct = (user) => {
        fetch(`https://furniture-server-site.vercel.app/users/${user._id}`, {
            method: 'DELETE',
            // headers: {
    ...
    const handleAdminBtn = (email) => {
        fetch(`https://furniture-server-site.vercel.app/users/admin/${email}`, {
            method: 'PUT',
        })
            .then(res => res.json())
            .then(data => {
